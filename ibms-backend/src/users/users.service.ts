...
import { ROLE_LEVEL, canAssignRole } from '../common/constants/role-hierarchy.js';
...
export class UsersService {
    ...
    async update(
        ...
        const isAdmin = (ROLE_LEVEL[currentUserRole] ?? 0) >= (ROLE_LEVEL['ADMIN'] ?? 0);
        ...
        } else if (dto.role !== undefined || dto.branchId !== undefined || dto.isActive !== undefined) {
    ...
    async deactivate(id: string, tenantId: string, currentUserId: string) {
        if (id === currentUserId) {
            throw new HttpException('Cannot deactivate yourself', HttpStatus.BAD_REQUEST);
        }

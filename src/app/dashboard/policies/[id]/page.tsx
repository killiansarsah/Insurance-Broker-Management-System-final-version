...
export default async function PolicyDetailPage({ params }: { params: Promise<{ id: string }> }) {
    ...
    const policy = {
        ...policyTemplate,
        ...rawPolicy,
        // Override template defaults if rawPolicy has them (it doesn't have timeline/docs yet)
        timeline: policyTemplate.timeline.map(t => ({ ...t })), // simple clone
    };
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div className="space-y-6">
                    ...
                    <Card className="p-5">
                        ...
                        <div className="flex items-center gap-3 mb-4">
                            <div className="w-10 h-10 rounded-lg bg-surface-100 flex items-center justify-center text-surface-900">
                                <Building2 size={20} />
                            </div>
                            <div>
                        ...
                        <Button variant="ghost" size="sm" className="w-full text-xs h-8">Contact Support</Button>
                    </Card>
                    ...
                    <Card className="p-5 overflow-hidden relative">
                        ...
                        <div className="flex items-center justify-between mb-2">
                            ...
                            <span className="text-sm font-bold text-success-600">{formatCurrency(policy.commissionReceived || 0)}</span>
                        </div>
                        <div className="w-full h-2 bg-surface-100 rounded-full overflow-hidden mb-1">
                            <div className="h-full bg-success-500 w-[60%]" />
                        </div>
                        <div className="flex justify-between text-[11px] text-surface-400 mb-4">
                            <span>{Math.round(((policy.commissionReceived || 0) / policy.commissionAmount) * 100)}% paid</span>
                            <span>of {formatCurrency(policy.commissionAmount)}</span>
                        </div>
                        ...
                        <div className="p-2 bg-accent-50 rounded-lg border border-accent-100">
                            <div className="flex justify-between items-center">
                                ...
                                <span className="text-sm font-bold text-accent-700">
                                    {formatCurrency(policy.commissionAmount - (policy.commissionReceived || 0))}
                                </span>
                            </div>
                        </div>
                    </Card>

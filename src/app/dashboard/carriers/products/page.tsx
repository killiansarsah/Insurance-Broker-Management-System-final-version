...
export default function ProductsPage() {
  ...
  const { data: carriersData, isLoading } = useAllCarriers();
  const carriers: Carrier[] = ((carriersData as any)?.items ??
    (carriersData as any)?.data ??
    (Array.isArray(carriersData) ? carriersData : [])) as Carrier[];
  ...
  const allCategories = useMemo(() => {
    ...
    carrierProducts.forEach((p: any) => {
      const cat = (p.insuranceType || "other").toLowerCase();
      cats.add(cat);
    });
    return Array.from(cats);
  }, [carrierProducts]);

  const compulsoryCount = carrierProducts.filter(
    (p: any) => p.compulsory,
  ).length;

  const filtered = carrierProducts.filter((p: any) => {
    const pCategory = (p.insuranceType || "other").toLowerCase();
    if (selectedCarrier !== "all" && p.carrierId !== selectedCarrier)
      return false;
    if (
      selectedCategory !== "all" &&
      pCategory !== (selectedCategory as string).toLowerCase()
    )
      return false;
  ...
  return (
    <div className="space-y-6 animate-fade-in">
        ...
        <div>
          <div className="flex items-center gap-2 mb-1">
            <Link
              ...
              className="text-xs font-bold text-surface-500 hover:text-primary-600 transition-colors uppercase tracking-widest"
            >
              Carriers
            </Link>
            <span className="text-surface-300">/</span>
            <span className="text-xs font-bold text-surface-800 uppercase tracking-widest">
              Products
            </span>
          </div>
          ...
          <p className="text-sm text-surface-500 mt-1">
            All insurance products across {carriers.length} carriers ·{" "}
            {carrierProducts.length} products listed.
          </p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {kpis.map((kpi, i) => (
          <Card key={i} padding="none" className="p-5">
            ...
            <p
              className={cn("text-2xl font-black mt-1 tabular-nums", kpi.color)}
            >
              {kpi.value}
            </p>
          </Card>
          ...
          <div className="w-full md:w-48">
            <CustomSelect
              options={[
                ...
                ...carriers.map((c: Carrier) => ({ label: c.shortName || c.name, value: c.id }))
              ]}
              value={selectedCarrier}
              onChange={(v) => setSelectedCarrier(String(v || 'all'))}
          ...
          <div className="w-full md:w-48">
            <CustomSelect
              options={[
                ...
                ...allCategories.map((cat: string) => ({ label: CATEGORY_LABELS[cat] || cat, value: cat }))
              ]}
              value={selectedCategory}
              onChange={(v) => setSelectedCategory(String(v || 'all') as "all" | ProductCategory)}
      ...
      <AnimatePresence mode="wait">
        <motion.div
          key={
            search + selectedCarrier + selectedCategory + String(compulsoryOnly)
          }
          ...
          {filtered.length === 0 ? (
            ...
            filtered.map((product: any, i: number) => {
              const carrier = carriers.find(
                (c: Carrier) => c.id === product.carrierId,
              );
              return (
                <motion.div
                  key={product.id}
                    ...
                    <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                      <div className="flex-1 min-w-0">
                        <div className="flex flex-wrap items-center gap-2 mb-1.5">
                          ...
                          <span
                            className={cn(
                              ...
                              CATEGORY_COLORS[
                                (product.insuranceType || "other").toLowerCase()
                        ...
                        <div className="flex flex-wrap gap-x-4 gap-y-1">
                          {(product.coverageSummary || []).map(
                            (cov: string, ci: number) => (
                              <span
                                key={ci}
